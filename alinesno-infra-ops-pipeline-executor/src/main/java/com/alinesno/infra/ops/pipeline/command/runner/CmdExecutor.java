...
package com.alinesno.infra.ops.pipeline.command.runner;
...
import com.alinesno.infra.ops.pipeline.command.utils.CommandUtil;
import com.alinesno.infra.ops.pipeline.command.utils.DateUtil;
import com.alinesno.infra.ops.pipeline.command.utils.SystemUtil;
    ...
    private String getExecutor() {
        if (SystemUtil.isWindows()) {
            ...
            if (Strings.isNullOrEmpty(cygwinHome)) {
                return CommandUtil.Windows.CMD_EXECUTOR;
            }
        ...
        if (SystemUtil.isMac()) {
            return CommandUtil.Mac.CMD_EXECUTOR;
        }

        return CommandUtil.Unix.CMD_EXECUTOR;
    }
        ...
        return () -> {
            try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(outputStream))) {
                for (String cmd : cmdList) {
                    writer.write(cmd + CommandUtil.Unix.LINE_SEPARATOR);
                    writer.flush();
                }

                // find env and set to result output if output filter is not null or empty
                if (!outputEnvFilters.isEmpty()) {
                    writer.write(String.format("echo %s" + CommandUtil.Unix.LINE_SEPARATOR, endTerm));
                    writer.write("env" + CommandUtil.Unix.LINE_SEPARATOR);
                    writer.flush();
                }
    ...
    private void readEnv(final BufferedReader reader,
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (index == -1 && value != null) {
                value.append(CommandUtil.Unix.LINE_SEPARATOR + line);
            }
        }
    }
